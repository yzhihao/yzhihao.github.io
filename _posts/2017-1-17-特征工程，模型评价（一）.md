...
{:toc}
	...
	在大二上学期，从javaweb转到机器学习，很久没有发博文了，我想说不是因为我懒，而是因为我真的发不了，没有能力发，从大二12月到现在，经过一段时间的学习，我把自己学过的笔记，结合自己的理解，尽量从最简单的角度看这些咋一看很复杂的机器学习算法。肯定错误会很多，希望大神们谅解我这个小白（逃），也希望可以通过写博文，而不是简单的学习笔记，可以加深自己对机器学习的印象！
	下面开始吧！
	机器学习，先贴下知乎的几句话，感觉特别赞同--：
	机器学习是什么？给你一堆数据，你猜一下大概是怎么一回事，交给电脑让它算算算看看是不是像这么一回事。那么为什么叫这个名字，不叫“计算机协助下的统计学假设检验与分布函数搜索”？因为听起来酷炫，好骗钱。学了机器学习，才知道，名字可以起的如此装逼。。。
	下面我从数据处理的方式将特征选择的一些方法，然后在讲下降维技术，特征提取，最后说下模型评估的那些事!
...
>[特征工程怎么做](http://www.jianshu.com/p/35135ab0a627)，这篇博文也值得借鉴
...
## 简单实践
讲到特征选择，最近有看下天池的一个比赛:[Repeat Buyers Prediction](https://tianchi.shuju.aliyun.com/getStart/introduction.htm?spm=5176.100066.333.1.1Ar5Ir&raceId=231576)，也是现在大三大佬们的大数据作业，任务就是根据用户3个月在天猫的行为日志，建立用户的品牌偏好，并预测他们在将来一个月内对品牌下商品的购买行为。在作业中老师给的特征只是用户之前的行为，所以最终效果真的有点差，百分之五点多的准确率，百分之五点多的召回率，百分之五点多的f1得分，嗯，我纯靠猜都可能比它强吧。。。
...
>在我的项目中，有用到卡方检验来提取特征词，然后来减少无关特征，达到特征选择的目的
...
![](http://img.blog.csdn.net/20160105195325068)
信息熵图：

![](http://i4.qhimg.com/t019668f2440a3a50ae.jpg)
...
![](http://img.blog.csdn.net/20160106010725183)
...
>咦，看到H(Y|X)就想起条件概率，其实，条件熵可以类比条件概率，他表示：在随机变量 X 的基础上我们引入随机变量 Y，假设 Y 和 X 有一定的关系。那么 Y 的信息熵会相对减小。
...
* 数学公式：
* 
![](https://segmentfault.com/image?src=http://e.hiphotos.baidu.com/album/pic/item/78310a55b319ebc413721b158226cffc1e171637.jpg&objectId=1190000000641079&token=86cbd929e4ea6dcdccc1f0aed315b11b)
...
![](http://images.cnitblog.com/blog/571227/201501/072030106714463.png)
...
然后是KL散度的期望=互信息
![](http://upload.wikimedia.org/math/b/f/5/bf5342166c3e1ddb8835a59bdddde43a.png)
...
![](http://img.blog.csdn.net/20161119134326973)
...
![](http://nbviewer.jupyter.org/github/zlotus/notes-LSJU-machine-learning/blob/master/resource/chapter03_image01.png)
...
![](http://blog.codinglabs.org/uploads/pictures/accurately-measuring-model-prediction-error/ModelError.png)
...
![](http://img.blog.csdn.net/20161204193150016)
...
损失函数的期望是![](http://images0.cnblogs.com/blog/790160/201507/261025515295675.png)
...
1. 合适的拟合：
2. 
![](http://52opencourse.com/?qa=blob&qa_blobid=14254427642975728120)
...
2. 过拟合：
![](http://52opencourse.com/?qa=blob&qa_blobid=13449125327891255846)
...
![](http://52opencourse.com/?qa=blob&qa_blobid=9287538657732644759)
...
### 频率学派-（ch11）
频率学派（frequentist）的观点看，θ是一个未知的但却是常量的值。对于频率派来说，θθ并不是随机变量，它是一个参数，只是恰巧不知道它的值而已，而我们的任务就是通过使用一些统计步骤（如最大似然法）来估计这个参数的值。
...
3. 
![](http://img.blog.csdn.net/20161119134410862)
...
这次就讲到这里吧，之后我会继续写好下面的博文，希望在写博文的同时，可以自我学习，一起加油！！！
